...
_Bool Disp_E=0;
volatile unsigned int TimerSalTriac=0,TimerBlink=500,TimerMultiplexacion=2;
uint8_t Duty=0,ValorTimerTriac=0;
volatile uint8_t DigitoActivo=1;
...
void Control(void)
{
    ...
    if(PORTBbits.RB2 && Duty < 100) Duty+=10,Antirrebote();
    else if(PORTBbits.RB4 && Duty > 10) Duty-=10,Antirrebote();
    else if(PORTBbits.RB3) Disp_E=!Disp_E,Antirrebote();
...
void CargaDigitos(void)
{
    ...
    if(Disp_E)
    {
        switch(DigitoActivo)
        {
            ...
            case 2:
            {
                PORTBbits.RB5=0,PORTBbits.RB6=1,PORTBbits.RB7=0;
                PORTA &= 0b11110000;
                PORTA |= decena;
            break;
            }
            case 3:
            {
                PORTBbits.RB5=0,PORTBbits.RB6=0,PORTBbits.RB7=1;
                PORTA &= 0b11110000;
                PORTA |= unidad;
            break;
            }
    ...
    else PORTBbits.RB5=0,PORTBbits.RB6=0,PORTBbits.RB7=0;
...
void Antirrebote(void)
{
    ...
    while(PORTBbits.RB3 || PORTBbits.RB2 || PORTBbits.RB4) _delay((unsigned long)((5)*(4000000/4000.0)));
...
void __attribute__((picinterrupt(("")))) ISR(void)
{
    ...
    if(TMR1IF == 1)
    {
        ...
        if(SalTriac_E==1 && TimerSalTriac!=0) TimerSalTriac--;
        if(TimerBlink!=0) TimerBlink--;
        if(TimerMultiplexacion!=0) TimerMultiplexacion--;
