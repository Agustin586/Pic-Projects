...
void E_pant1_Lcd(void)
{
    ...
    if(mostrar == 0)
    {
        LCD_array(1,1,"ID-PIC:");
        LCD_array(3,1,"Hora:"),LCD_array(4,1,"Fecha:");
        mostrar = 1;
        M_variable = 1;
    }
    ...
    if(!Timer_DelayMuestra)
    {
        if(mostrar)
        {
            ...
            LCD_array(3,7,buffer),LCD_array(3,9,":");

            sprintf(buffer,"%02d",min);
            LCD_array(3,10,buffer),LCD_array(3,12,":");

            sprintf(buffer,"%02d",seg);
            LCD_array(3,13,buffer);

            sprintf(buffer,"%02d",dia);
            LCD_array(4,7,buffer),LCD_array(4,9,"/");

            sprintf(buffer,"%02d",mes);
            LCD_array(4,10,buffer),LCD_array(4,12,"/");
...
void E_pant2_Lcd(void)
{
    ...
    if(!Timer_DelayMuestra)
    {
        if(mostrar == 0)
        {
            ...
            LCD_array(1,1,"CARD:");
            LCD_array(2,1,"USUARIO:");
            LCD_array(4,1,"Cant.Usuarios:");
            mostrar = 1;
        }
            ...
            case 1:
            {
                ...
                while(1)
                {
                    ...
                    if(codigo_rfid == 0)
                    {
                        ...
                        LCD_array(4,6,buffer);
                    }


                    if(aceptar == 1) LCD_command(0x01),paso+=1,valor=0,aceptar=0;
                    ...
                    while(paso == 2)
                    {
                        ...
                        if((User_Scan(num_usuario)==0) && (Read_Hash_ex(codigo_rfid) == 0))
                        {
                            ...
                            ubicacion = New_User_Pos();
                            Write_User(ubicacion,num_usuario);
                            _delay((unsigned long)((20)*(48000000/4000.0)));
                            M_Write((ubicacion+3),codigo_rfid);
                            _delay((unsigned long)((20)*(48000000/4000.0)));
                            Sum_Cant_Usuarios();
                            _delay((unsigned long)((20)*(48000000/4000.0)));
                            ...
                            LCD_command(0x01),LCD_array(2,1,"  USUARIO AGREGADO  ");
                        ...
                        {
                            LCD_command(0x01),LCD_array(2,1,"  USUARIO EXISTENTE "),_delay((unsigned long)((1000)*(48000000/4000.0))),valor=0,mostrar=0;
                            Fin_Config=1;
                            paso +=1;
