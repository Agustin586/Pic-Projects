...
void __attribute__((picinterrupt(("low_priority")))) ISR_LowP(void)
{
    ...
    if(INTCONbits.TMR0IF == 1)
    {
        ...
        if(Timer_Task1 != 0 && !Fin_Cad) Timer_Task1--;
        if(Timer_Task2 != 0 && !Fin_Cad) Timer_Task2--;


        if(Timer_Tec != 0 && !Fin_Cad) Timer_Tec--;
        if(Timer_Rtc != 0 && mod_rtc == 0 && !Fin_Cad) Timer_Rtc--;


        if(Timer_DelayLcd != 0) Timer_DelayLcd--;
        if(Timer_DelayMuestra != 0) Timer_DelayMuestra--;


        if(Timer_Puerta1 != 0 && puerta1) Timer_Puerta1--;
        if(Timer_Puerta2 != 0 && puerta2) Timer_Puerta2--;


        if(Tarj_Dect == 1 && Timer_Hash != 0) Timer_Hash --;
